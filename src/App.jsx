      ...
      <div className="max-w-4xl w-full text-center space-y-12 animate-fade-in-up">
        ...
        <div className="grid md:grid-cols-3 gap-6 max-w-4xl mx-auto">
          <button onClick={() => setPage("data")} className="group p-6 bg-white rounded-3xl border border-slate-100 shadow-xl hover:shadow-2xl hover:-translate-y-1 transition-all text-left">
            ...
            <p className="text-slate-500 text-sm">เพิ่มคำศัพท์ (ต่อเนื่อง 60 เฟรม)</p>
          </button>
      ...
      if (showSkeleton) {
        for (const handLandmarks of results.landmarks) {
            for (let p of handLandmarks) {
                ...
                // เปลี่ยนสีเมื่อกำลังอัด Recording
...
// ==============================================
// 🟢 DataPage (แก้ไขหลักตรงนี้สำหรับการบันทึก 60 เฟรม)
// ==============================================
function DataPage() {
  const [currentLandmarks, setCurrentLandmarks] = useState(null); // ไว้โชว์เฉยๆ
  const [label, setLabel] = useState("");
  const [status, setStatus] = useState("ready"); // ready, recording, uploading, success
  const [recordedFrames, setRecordedFrames] = useState([]);
  
  // ใช้ useRef เพื่อเก็บข้อมูลระหว่างเฟรมโดยไม่ต้อง Re-render บ่อยเกินไป
  ...
  const handleLandmarksUpdate = (points) => {
    ...
    if (status === "recording") {
        if (points) {
            ...
            // อัปเดต UI เพื่อแสดงจำนวนเฟรม (ใช้ length ของ buffer)
            setRecordedFrames([...framesBuffer.current]);

            // เช็คว่าครบ 60 เฟรมหรือยัง
  ...
  const finishRecording = async () => {
    ...
    try {
      // ส่งข้อมูลไปเป็น sequences (array ของ arrays)
      const res = await fetch(`${API_URL}/upload`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ 
            label: label, 
            sequences: finalData // เปลี่ยน key จาก points เป็น sequences เพื่อสื่อว่าเป็นภาพต่อเนื่อง
        })
      });
      ...
      alert("ไม่สามารถเชื่อมต่อ Server ได้ หรือ Server ยังไม่รองรับข้อมูลแบบต่อเนื่อง");
      console.error(e);
      setStatus("ready");
    }
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col lg:flex-row gap-10 animate-fade-in">
      <div className="flex-1 space-y-4">
        ...
        <CameraView 
          ...
          overlayText={currentLandmarks ? `ตรวจพบ ${currentLandmarks.length / 63} มือ` : "กรุณายกมือ"}
        />
      </div>
          ...
          <div className="relative z-10 space-y-8">
            <div>
              ...
              <p className="text-slate-500 mt-2 text-sm leading-relaxed">
                ระบบจะบันทึกภาพต่อเนื่อง <strong>60 เฟรม</strong> เมื่อกดปุ่มเริ่ม กรุณาทำท่าทางต่อเนื่องจนกว่าจะครบ
              </p>
            </div>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                {status === "success" ? "บันทึกเรียบร้อย" : 
                 status === "recording" ? `กำลังบันทึก... (${recordedFrames.length}/60)` :
            ...
            <div className="bg-blue-50/50 p-4 rounded-2xl border border-blue-100 text-sm text-blue-700/80">
              <div className="flex gap-3">
                 ...
                 <p>กรุณาทำท่าทางให้ <strong>ต่อเนื่อง</strong> และ <strong>สมูท</strong> เพื่อให้ AI เรียนรู้ลักษณะการเคลื่อนไหวได้ดีที่สุด</p>
              </div>
            </div>
          </div>
        </div>
...
function PredictPage() {
  ...
  // สำหรับการทำนาย ก็อาจจะต้องใช้ Buffer เช่นกันถ้า Model ต้องการ Sequence
  // แต่เบื้องต้นปล่อยเป็น Single Frame ไว้ก่อน หรือถ้าโมเดลต้องการ 60 เฟรม
  // จะต้องแก้ Logic ตรงนี้คล้าย DataPage ครับ
  ...
  const handleLandmarks = async (points) => {
    ...
    try {
      const res = await fetch(`${API_URL}/predict`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ points }) // ถ้า Backend เปลี่ยนรับ Sequence ต้องแก้ตรงนี้ด้วย
      });
