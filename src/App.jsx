...
import { 
  ...
  ChevronRight, HeartPulse, Brain, BookOpen, Layers, X
} from "lucide-react";
...
function DataPage() {
  ...
  const [existingLabels, setExistingLabels] = useState([]); // เก็บชื่อท่าเก่า

  // ดึงข้อมูลชื่อท่าเก่าๆ มาแสดง
  useEffect(() => {
    fetch(`${API_URL}/dataset`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // *** จุดสำคัญ: ไม่ต้อง setLabel("") เพื่อให้ชื่อค้างอยู่ ***
      // setLabel(""); <--- ลบบรรทัดนี้ออก
          ...
          <div className="relative z-10 space-y-8">
            <div>
              ...
              <p className="text-slate-500 mt-2 text-sm leading-relaxed">
                ทำท่าทางค้างไว้ แล้วกดบันทึก (ชื่อท่าจะไม่หายไป เพื่อให้บันทึกซ้ำได้สะดวก)
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                {existingLabels.length > 0 && (
                  <div className="mt-3">
                    <p className="text-xs text-slate-400 mb-2">เลือกจากที่มีอยู่:</p>
                    <div className="flex flex-wrap gap-2 max-h-32 overflow-y-auto custom-scrollbar p-1">
                      {existingLabels.map((l, idx) => (
                        <button
                          key={idx}
                          onClick={() => setLabel(l)}
