...
function DictionaryPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState(null); // sign name to delete
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true);
    try {
      await fetch(`${API_URL}/delete-sign/${encodeURIComponent(deleteTarget)}`, {
        method: "DELETE",
      });
      closeDelete();
      fetchSigns();
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center px-4">
          <div className={`bg-white rounded-3xl shadow-2xl p-8 w-full max-w-sm ${pwShake ? "animate-[wiggle_0.4s_ease-in-out]" : ""}`}>
            ...
            <p className="text-slate-500 text-sm text-center mb-1">
              ลบท่า <strong className="text-red-600">"{deleteTarget}"</strong> ทั้งหมด?
            </p>
            ...
            <div className="space-y-3">
              <input
                ...
                className={`w-full px-4 py-3.5 rounded-2xl border text-sm font-medium outline-none transition-all ${
                  pwError
                    ...
                    : "border-slate-200 bg-slate-50 focus:ring-2 focus:ring-red-300/30 focus:border-red-300"
                }`}
              />
              {pwError && (
              ...
              <div className="flex gap-3">
                <button
                  onClick={closeDelete}
                  className="flex-1 py-3 rounded-2xl border-2 border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition-all"
                >
                  ยกเลิก
                </button>
                <button
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-slate-500 mt-1 text-sm">
            ท่าทางทั้งหมดที่ AI เรียนรู้แล้ว — {signs.length} ท่า
          </p>
        </div>
        {signs.length > 0 && (
          <input
            ...
            return (
              <div key={item.name} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all group">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    {item.has_motion && (
                      <span className="text-xs bg-amber-100 text-amber-600 px-2 py-0.5 rounded-full font-medium">🤚 ขยับ</span>
                    )}
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                      total >= 20 ? "bg-emerald-100 text-emerald-700" : total >= 10 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-600"
                    }`}>
                      {total} ตย.
                    </span>
                  </div>
                ...
                {item.steps > 1 && (
                  <p className="text-xs text-purple-500 font-medium mb-2">{item.steps} ขั้นตอน</p>
                )}
