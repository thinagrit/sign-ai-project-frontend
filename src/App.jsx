...
function PredictPage() {
  ...
  const stepColors = ["from-blue-500 to-blue-600","from-purple-500 to-purple-600","from-teal-500 to-teal-600"];
  ...
  const handleLandmarksAuto = async (points) => {
    ...
    try {
      ...
      const res = await fetch(`${API_URL}/predict-step`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ label: String(step), points }),
      });
      if (!res.ok) throw new Error();
      const data = await res.json();
      setServerError(false);

      const expectedLabel = totalSteps === 1 ? selectedSign.name : `${selectedSign.name}_${step}`;

      if (data.label === expectedLabel && data.confidence > 0.6) {
        const newResults = { ...stepResultsRef.current, [step]: data };
        setStepResults(newResults);
        stepResultsRef.current = newResults;
        ...
        if (step < totalSteps) {
          ...
          const result = { name: selectedSign.name, confidence: avgConf };
      ...
      {mode === "auto" && (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
          <p className="text-sm font-semibold text-slate-600 mb-3">
            {signs.length > 0 ? "เลือกท่าที่จะแปล:" : "กำลังโหลดรายการท่า..."}
          </p>
          {signs.length > 0 ? (
            <div className="flex flex-wrap gap-2">
              {signs.map((s) => (
                ...
                >
                  {s.name}
                  {s.steps > 1 && (
                    <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${selectedSign?.name === s.name ? "bg-white/20 text-white" : "bg-purple-100 text-purple-600"}`}>
                      {s.steps} ขั้น
                    </span>
                  )}
                  {s.has_motion && (
                    <span className={`text-xs ${selectedSign?.name === s.name ? "opacity-70" : "text-amber-500"}`}>🤚</span>
                  )}
                </button>
              ))}
            </div>
        ...
        <div className="lg:col-span-3">
          <CameraView
            ...
            overlayText={
              ...
              mode === "auto" && selectedSign ? `ทำท่า: ${selectedSign.name}${totalSteps > 1 ? ` — ขั้น ${currentStep}/${totalSteps}` : ""}` :
              mode === "auto" ? "เลือกท่าก่อน" :
                  ...
                  return (
                    <div key={step} className={`flex items-center gap-3 p-3 rounded-xl transition-all ${active ? "bg-blue-50 border border-blue-100" : done ? "bg-emerald-50 border border-emerald-100" : "bg-slate-50 border border-slate-100"}`}>
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs font-mono text-slate-400">{selectedSign.name}{totalSteps > 1 ? `_${step}` : ""}</p>
                      </div>
                      {done && <span className="text-xs text-emerald-600 font-bold">{Math.round(done.confidence*100)}%</span>}
                      {active && !done && <span className="text-xs text-blue-500 font-bold animate-pulse">รอ...</span>}
                    </div>
                  );
                })}
              </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-lg p-6 flex-1">
            ...
            {finalResult && mode === "auto" ? (
              <div className="text-center py-4">
                ...
                <p className="text-emerald-600 font-bold text-sm">ความมั่นใจ {Math.round(finalResult.confidence*100)}%</p>
              </div>

            /* Waiting for step (auto) */
              ...
              <div className="text-center py-4">
                ...
                <p className="text-slate-500 text-sm">
                  ทำท่า <strong>{selectedSign.name}{totalSteps > 1 ? `_${currentStep}` : ""}</strong>
                </p>
                <p className="text-slate-400 text-xs mt-1">ค้างท่าให้ AI จับภาพ</p>
              </div>

            /* No sign selected (auto) */
            ) : mode === "auto" ? (
              <div className="text-center py-6">
                <div className="text-4xl mb-3">👆</div>
                <p className="text-slate-400 text-sm">เลือกท่าด้านบนก่อนเริ่มแปล</p>
              </div>
